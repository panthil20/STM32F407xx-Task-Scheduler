...
static void init_systick_timer(uint32_t tick_hz); // Context switch time
...
static void task_delay(uint32_t tick_count);
...
{
	uint32_t psp_value;
	uint32_t block_count; // time till task is in blocked state
...
int main(void)
{
	...
	init_systick_timer(TICK_HZ); // Round Robin Scheduling
...
__attribute__((naked)) void switch_sp_to_psp(void)
{
	...
	//2. change SP to PSP using CONTROL register SPSEL register's 2nd bit
...
void task_delay(uint32_t tick_count) // puts task to blocked state for specified delay
{
	//disable interrupt
	INTERRUPT_DISABLE();
...
void  SysTick_Handler(void)
{
    ...
    update_global_tick_count(); // increase by 1 as systick exception will occur
